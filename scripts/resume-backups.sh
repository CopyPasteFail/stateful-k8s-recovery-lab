#!/usr/bin/env bash
# resume-backups.sh — set spec.suspend=false on the backup CronJob.
set -euo pipefail
...
printf '%s\n' '' '=== stateful-k8s-recovery-lab: resume-backups ==='
...
if [[ "${SUSPENDED}" != "true" ]]; then
    ok "CronJob '${BACKUP_CRONJOB_NAME}' is already active — nothing to do."
    printf '\n'
    exit 0
fi
...
info "Next scheduled run: $(kubectl get cronjob "${BACKUP_CRONJOB_NAME}" \
    -n "${NS_APP}" -o jsonpath='{.spec.schedule}' 2>/dev/null || printf 'unknown')"
